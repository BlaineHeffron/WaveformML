    ...
    def _read_chunk(self, file_info, dataset, columns):
        ...
        with h5py.File(file_info[0], 'r', ) as h5_file:
            ...
            if info['n_events'] - 1 != file_info[1][1]:
                ...
                if file_info[1][0] > 0:
                    ...
                    inds = None
        if labels is None and self.label_file_pattern:
        ...
        if inds:
            if labels is not None:
                if len(columns) == 3:
                    return coords[inds], features[inds], labels[inds]
...
class PulseDatasetWFPairNorm(PulseDataset):
    """Pulse data in the form of ChannelData of size [N,nsamples*2],
    where N is the number of PMTs fired for the M = batch size events
    contains z which is the true z value and E which is the true E value"""
...
class PulseDatasetWaveformNorm(PulseDataset):
    """Pulse data in the form of ChannelData of size [N,nsamples],
    where N is the number of PMTs fired for the M = batch size events"""
...
class PulseDatasetNormFeatures(PulseDataset):
    """Pulse data in the form of ChannelData of size [N,nsamples],
    where N is the number of extracted waveform features for M = batch size events"""
