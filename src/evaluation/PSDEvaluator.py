import numpy as np
from src.utils.SparseUtils import average_pulse, find_matches, metric_accumulate_2d, metric_accumulate_1d, get_typed_list
...
class PSDEvaluator:
    ...
    def add(self, batch, output, predictions):
        (c, f), labels = batch
        c,f,labels,predictions = c.detach().numpy(),f.detach().numpy(),labels.detach().numpy(),predictions.detach().numpy()
        ...
        for i in range(self.n_classes):
            self.logger.experiment.add_histogram("evaluation/energy_{}".format(self.class_names[i]), extract_values(energy,labels,i))
            self.logger.experiment.add_histogram("evaluation/psd_{}".format(self.class_names[i]), extract_values(psd,labels,i))
            self.logger.experiment.add_histogram("evaluation/multiplicity_{}".format(self.class_names[i]), extract_values(multiplicity,labels,i),bins=np.arange(0.5,self.n_mult+0.5,1))
        ...
        metric_accumulate_1d(results, multiplicity, *self.results["mult_acc"], get_typed_list([0.5, self.n_mult + 0.5]), self.n_mult)
        """
        ...
        """
        metric_accumulate_2d(results, np.stack((energy, psd), axis=1), *self.results["ene_psd_acc"], get_typed_list([self.emin, self.emax]),
    ...
    def dump(self):
        self.logger.experiment.add_figure("evaluation/energy_psd_accuracy", plot_countour(self.calc_axis(self.emin, self.emax, self.n_bins),
        ...
        self.logger.experiment.add_figure("evaluation/position_accuracy", plot_countour(np.arange(1, self.nx + 1, 1), np.arange(1, self.ny + 1, 1),
        ...
        self.logger.experiment.add_figure("evaluation/multiplicity_accuracy",
            plot_bar(np.arange(1, self.n_mult + 1), self.results["mult_acc"][0][1:self.n_mult+1] /
                     self.results["mult_acc"][1][1:self.n_mult+1], "multiplicity", "accuracy"))
        self._init_results()
