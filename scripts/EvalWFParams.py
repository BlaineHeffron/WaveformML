...
class WFParamEvaluator(WFParamsDB):
    ...
    def best_fits_per_seg(self, calname, printed_params=None):
        ...
        for seg in range(14*11):
            result = self.query_smallest_diffs(calname, seg, printed_params, 1)
            if result:
                for row in result:
...
def main():
    ...
    wfpe.eval_wf_params()
    wfpe.best_fits_per_seg(args.calname)
