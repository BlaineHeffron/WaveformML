...
def get_from_path(obj, name):
    ...
    if m.group():
        ...
        return getattr(obj, name[0:m.start()])
...
class ModelOptimization:
    ...
    def __init__(self, optuna_config, config, model_dir, trainer_args):
        ...
        self.trainer_args = DictionaryUtility.to_object(trainer_args)
    ...
    def parse_config(self):
        ...
        for h in self.optuna_config.hyperparameters.keys():
            myobj = None
            for i, name in enumerate(h.split("/")):
                if i > 0:
    ...
    def modify_config(self, trial):
        for hp in self.hyperparameters.keys():
            name = hp.split("/")[-1]
            bounds = self.optuna_config.hyperparameters[hp]
